...
namespace Pathfinding {
	...
	public abstract class AIBase : VersionedMonoBehaviour {
		...
		public MovementType movementType = MovementType.Auto;
		public UnityAction<Vector3> moveAction;
			...
			if (controller != null && controller.enabled && updatePosition) {
				...
				switch (movementType) {
					...
					case MovementType.Custom:
						moveAction?.Invoke((nextPosition - currentPosition + accumulatedMovementDelta).normalized);
						break;
				}
			...
			if ((positionDirty1 || positionDirty2) && updatePosition) {
				...
				switch (movementType) {
					case MovementType.Auto:
						...
						break;
					case MovementType.Custom:
						moveAction?.Invoke((nextPosition - gameObject.transform.position).normalized);
						break;
				}
			}
