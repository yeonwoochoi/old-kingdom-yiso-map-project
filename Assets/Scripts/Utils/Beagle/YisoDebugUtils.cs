...
namespace Utils.Beagle {
    public static class YisoDebugUtils {
            ...
            get {
                ...
                for (int i = 0; i < LogHistory.Count; i++) {
                    ...
                    log.Append("<color=#f9a682>[" +
                               FloatToTimeString(LogHistory[i].Time, false, true, true, true) + "]</color> ");
        ...
        public static void DebugLogTime(object message, string color = "", int timePrecision = 3,
            ...
            output += "<color=#f9a682>[" + FloatToTimeString(Time.time, false, true, true, true) +
                      "]</color> ";
        ...
        /// <param name="cubeSize"></param>
        /// <param name="wireOnly"></param>
        public static void DrawGizmoArc(Transform transform, float angle, float radius, bool wireOnly) {
            if (!DebugDrawEnabled) {
                return;
            }
            ...
            var from = new Vector3(Mathf.Cos(angle / 2 * Mathf.Deg2Rad), -Mathf.Sin(angle / 2 * Mathf.Deg2Rad), 0f)
                .normalized * radius;
            if (wireOnly) {
                Handles.DrawWireArc(transform.localPosition, Vector3.forward, from, angle, radius, 1f);
            }
            else {
                Handles.DrawSolidArc(transform.localPosition, Vector3.forward, from, angle, radius);
            }
#endif
        }
        ...
        public static string FloatToTimeString(float t, bool displayHours = false, bool displayMinutes = true,
            ...
            return displayHours switch {
                true when displayMinutes && displaySeconds && displayMilliseconds =>
                    $"{hours:00}:{minutes:00}:{seconds:00}.{milliseconds:D3}",
                false when displayMinutes && displaySeconds && displayMilliseconds =>
                    $"{minutes:00}:{seconds:00}.{milliseconds:D3}",
                false when !displayMinutes && displaySeconds && displayMilliseconds =>
                ...
                false when displayMinutes && displaySeconds && !displayMilliseconds => $"{minutes:00}:{seconds:00}",
                _ => null
            };
        }
    }
